...
void	broadcastMsg(t_client *clients, int pos, int count)
{
	char	*extract;
	char	*msg;
	int	bytes;
	...
	while (extract_message(&clients[pos].buf, &extract) == 1)
	{
		...
		{
			free(extract);
			emit_error(-1, clients, count);
		}

		bytes = sprintf(msg, "client %d: %s", clients[pos].id, extract);
		broadcast(clients, msg, bytes, clients[pos].id, count);
		free(extract);
		free(msg);
	}
}
